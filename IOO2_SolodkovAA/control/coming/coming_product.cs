...
namespace IOO2_SolodkovAA
{
    ...
    public partial class coming_product : UserControl
	{
        public String ids = "";
        string id_coming = "";
        public coming_product(string idt)
		{
            id_coming = idt;
			InitializeComponent();
		}

        private void coming_product_Load(object sender, EventArgs e)
        {
            ...
            if (namess != "" && kol != "" && prices != "")
            {
                using (SqlConnection con = new SqlConnection(@"Data source=.\SQLEXPRESS;Initial Catalog=I002_Solodkov;User ID=user;Password=Passw0rd;"))
                {
                    try
                    {
                        ...
                        cmd.CommandText = "select * from pcoming where id_coming=" + id_coming+"AND id_product=" + ids;
                        ...
                        {
                            //заполнение данных
                            cmd.CommandText = "INSERT pcoming (id_coming, id_product,price,count) VALUES ('" + id_coming + "','" + ids + "','" + prices + "','" + kol + "')";
                            cmd.ExecuteScalar();
