...
{
	public partial class coming_add_kontroagent : UserControl
	{
        string ids;
		public coming_add_kontroagent(string id)
		{
            ids = id;
			InitializeComponent();
		}

		private void coming_add_kontroagent_Load(object sender, EventArgs e)
		{
		...
		{
            coming_new uc = new coming_new(ids);
                ...
                if (e.ColumnIndex == 1 || e.ColumnIndex == 2 || e.ColumnIndex == 3 || e.ColumnIndex == 4 || e.ColumnIndex == 5)
                {
                    ...
                    using (SqlConnection con = new SqlConnection(@"Data source=.\SQLEXPRESS;Initial Catalog=I002_Solodkov;User ID=user;Password=Passw0rd;"))
                    {
                        try
                        {
                            ...
                            SqlCommand cmd = con.CreateCommand();
                            //заполнение данных
                            cmd.CommandText = "UPDATE [coming] SET id_kontragent='" + id + "' Where [coming].id=" + ids;
                            cmd.ExecuteScalar();
